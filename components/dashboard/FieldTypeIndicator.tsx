...
export function FieldTypeIndicator({ type, size = 'sm' }: FieldTypeIndicatorProps) {
  const iconSize = size === 'sm' ? 'w-3 h-3' : 'w-4 h-4';

  const config: Record<FieldType, { icon: React.ReactNode; color: string; bg: string; label: string }> = {
    percentage: {
  ...
  return (
    <div className={`inline-flex items-center gap-1 px-2 py-1 rounded ${bg} ${color}`} title={label}>
      {icon}
