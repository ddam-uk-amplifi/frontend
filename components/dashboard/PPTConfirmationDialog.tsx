...
import { X, Download, Eye, BarChart3, GripVertical, Sparkles } from 'lucide-react';
...
function DroppableSlide({ slideNumber, graphsOnSlide, onDrop }: DroppableSlideProps) {
  const ref = useRef<HTMLDivElement>(null);
  ...
  return (
    ...
    >
      ...
      {/* Graph Content */}
      {hasGraphs && !isActive ? (
        <DraggableGraphCard graph={graphsOnSlide[0]} slideNumber={slideNumber} />
      ) : !hasGraphs && !isActive ? (
...
function PPTConfirmationDialogContent({
  ...
  const graphsBySlide = sortedGraphs.reduce((acc, graph) => {
      ...
      <div className="relative bg-gradient-to-br from-white to-slate-50 rounded-2xl shadow-2xl w-full max-w-5xl mx-4 max-h-[90vh] flex flex-col border border-slate-200/60 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200/60 bg-white/80 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-slate-800">PowerPoint Report Preview</h2>
              <p className="text-sm text-slate-500">
                Review your {selectedGraphs.length} graph{selectedGraphs.length !== 1 ? 's' : ''} and slide placements
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-auto p-6">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-violet-50 to-purple-50 border border-violet-200 rounded-xl p-4 flex items-center gap-3">
                ...
                <p className="text-sm text-violet-800">
                  <strong>Drag & Drop:</strong> Click and drag graphs to move them between slides
                </p>
              </div>
              ...
              <div className="bg-slate-50 rounded-xl p-4">
                <ul className="space-y-3">
                  {sortedGraphs.map((graph, index) => (
                    <li key={graph.id} className="flex items-center justify-between gap-3 p-4 bg-white rounded-xl border border-slate-200/60 shadow-sm">
                      <div className="flex items-center gap-3 flex-1">
                        ...
                        <div className="flex-1">
                          <div className="text-slate-800 font-medium">{graph.title}</div>
                        </div>
                      </div>
                      {graph.slideNumber ? (
          ...
          <div className="mt-6 bg-gradient-to-r from-sky-50 to-blue-50 border border-sky-200 rounded-xl p-4">
            <p className="text-sm text-sky-800">
              <strong>Tip:</strong> Graphs with assigned slide numbers will be placed at those specific positions. 
              Unassigned graphs will be added at the end of your presentation.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200/60 bg-white/80 backdrop-blur-sm rounded-b-2xl">
          <div className="text-sm text-slate-600">
            <span className="font-semibold text-violet-600">{graphsWithSlides.length}</span> assigned · 
            <span className="font-semibold text-amber-600 ml-1">{graphsWithoutSlides.length}</span> unassigned
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={onClose}
